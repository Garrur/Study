...
exports.login = async (req, res) => {
    try {
        ...
        const payload = {
            email: user.email,
            id: user._id,
            accountType: user.accountType, // Use this for route authorization
        };
        ...
        // Set cookie and send the response
        res.cookie('token', token, cookieOptions).status(200).json({
            success: true,
            message: 'User logged in successfully',
            user,  // Send the user object (without password)
        ...
        res.status(500).json({
            ...
            error: error.message, // Send the error message to the client (safe to send)
        });
    }
};
